...
const BentoCard: React.FC<{ children: React.ReactNode; className?: string; title?: string; onClick?: () => void; noDefaultBg?: boolean }> = ({ children, className = "", title, onClick, noDefaultBg = false }) => {
    ...
    return (
        ...
        >
            ...
            <div className="pointer-events-none absolute -inset-px opacity-0 transition duration-500 z-10"
                style={{ 
                    opacity, 
                    background: `radial-gradient(600px circle at ${position.x}px ${position.y}px, rgba(251, 191, 36, 0.15), transparent 40%)` 
                }}
            />
            <div className="pointer-events-none absolute -inset-px opacity-0 dark:opacity-0 transition duration-500 z-10 dark:block hidden"
                 style={{ 
                    opacity, 
            ...
            <div className="relative h-full p-8 z-20 flex flex-col">
                {title && <h3 className="text-lg font-bold text-gray-900 dark:text-neon-text-primary mb-4 uppercase tracking-wider">{title}</h3>}
                {children}
            </div>
        </div>
    );
...
const ServiceItem: React.FC<{ service: Service }> = ({ service }) => (
    <div className="flex flex-col h-full justify-between">
        <div>
            ...
            <h4 className="text-xl font-bold text-gray-900 dark:text-neon-text-primary mb-3 group-hover:text-accent-yellow-dark dark:group-hover:text-neon-cyan transition-colors duration-300">{service.title}</h4>
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
            {service.tags?.map((tag, idx) => (
                <span key={idx} className="px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-gray-600 dark:text-neon-text-primary bg-gray-50 dark:bg-black border border-gray-200 dark:border-neon-border rounded-lg dark:rounded-none group-hover:border-accent-yellow/50 dark:hover:border-neon-cyan group-hover:text-gray-900 dark:hover:text-neon-cyan group-hover:bg-white dark:hover:bg-black transition-all duration-300 shadow-sm group-hover:shadow-md">
                    {tag}
                </span>
            ))}
        </div>
                    ...
                    <Tilt3D tiltMaxAngle={12} scale={1.02}>
                        <BentoCard className="flex flex-col justify-center items-center text-center bg-white dark:bg-black border-gray-100 dark:border-neon-cyan relative overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-neon-cyan group">
                            ...
                            <div className="w-full h-full flex flex-col justify-center items-center relative z-10 py-6">
                                ...
                                >
                                    5<span className="text-4xl align-top text-accent-yellow dark:text-neon-cyan">+</span>
                                </motion.div>
                                <div className="text-sm font-extrabold uppercase tracking-widest text-gray-500 dark:text-neon-text-secondary mb-8">Years Experience</div>
                                ...
                                >
                                    {[
                                        ...
                                        <motion.div
                                            key={idx}
                                            className="bg-white dark:bg-black border border-gray-200 dark:border-neon-cyan/50 rounded-xl dark:rounded-none px-4 py-2.5 text-sm font-bold text-gray-900 dark:text-white flex items-center gap-3 shadow-sm hover:shadow-md dark:shadow-[0_0_10px_rgba(6,182,212,0.2)] transition-all duration-300"
                                            variants={{
                                                hidden: { opacity: 0, x: -20 },
                                                visible: { opacity: 1, x: 0 },
                                            }}
                                            whileHover={{ scale: 1.05, backgroundColor: '#fbbf24', color: '#000000', borderColor: '#fbbf24' }}
                                        >
                         ...
                         <div className="flex flex-col lg:flex-row gap-10 items-center">
                            ...
                            <div className="w-full lg:w-1/3 flex flex-col sm:flex-row lg:flex-col gap-4">
                                 <motion.div 
                                    ...
                                    whileHover={{ scale: 1.02, borderColor: '#3b82f6', boxShadow: '0 10px 30px rgba(59, 130, 246, 0.15)' }}
                                 >
                                    <div className="w-14 h-14 rounded-xl bg-blue-50 dark:bg-neon-cyan/10 border border-blue-100 dark:border-neon-cyan flex items-center justify-center text-blue-600 dark:text-neon-cyan text-xl"><i className="fas fa-map-pin"></i></div>
                                    <div>
                                        <div className="text-[10px] font-bold text-gray-500 dark:text-neon-text-tertiary uppercase tracking-wider">Based in</div>
                                 ...
                                 <motion.div 
                                    ...
                                    whileHover={{ scale: 1.02, borderColor: '#22c55e', boxShadow: '0 10px 30px rgba(34, 197, 94, 0.15)' }}
                                 >
                                    <div className="w-14 h-14 rounded-xl bg-green-50 dark:bg-neon-purple/10 border border-green-100 dark:border-neon-purple flex items-center justify-center text-green-600 dark:text-neon-purple text-xl"><i className="fas fa-globe"></i></div>
                                    <div>
                                        <div className="text-[10px] font-bold text-gray-500 dark:text-neon-text-tertiary uppercase tracking-wider">Languages</div>
                        ...
                        {SERVICES.map((service, i) => (
                            ...
                            >
                                <Tilt3D tiltMaxAngle={10} scale={1.03}>
                                    <BentoCard noDefaultBg={true} className="min-h-[260px] bg-white dark:bg-black border border-gray-100 dark:border-neon-border hover:border-accent-yellow/50 dark:hover:border-neon-cyan group hover:shadow-[0_20px_40px_rgba(251,191,36,0.1)] dark:hover:shadow-neon-cyan">
